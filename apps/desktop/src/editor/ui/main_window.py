...
class MainWindow(QMainWindow):
    ...
    def _setup_central_widgets(self) -> None:
        ...
        self._action_insert_image = insert_menu.addAction(
            self.tr("Image..."), self._insert_image
        )
        ...
        # New tab shortcut: Qt maps the portable "Ctrl" text to Cmd on macOS
        # and to the literal Ctrl key on Windows/Linux, giving us Cmd+T /
        # Ctrl+T for free from a single QKeySequence.
    ...
    def _retranslate_ui(self) -> None:
        ...
        if hasattr(self, "_insert_menu"):
            self._insert_menu.setTitle(self.tr("Insert"))
        if hasattr(self, "_action_insert_image"):
            self._action_insert_image.setText(self.tr("Image..."))
    ...
    def _insert_image(self) -> None:  # pragma: no cover - UI wiring
        """Insert a `[image](path)[/image]` tag for a picked file at the cursor.

        Reuses the `.story` markup image tag already understood by
        story_markup.py — no new DSL, just a way to author it without typing
        the brackets by hand.
        """
        ...
        path_str, _ = QFileDialog.getOpenFileName(
            ...
            "",
            self.tr("Images (*.png *.jpg *.jpeg *.gif *.webp)"),
        )
        if not path_str:
            return

        tag = f"[image left width=40% height=20%]({path_str})[/image]"
